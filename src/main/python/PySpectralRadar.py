...
def createFreeformScanPattern(Probe, positions, size_x, size_y, apodization):
    ...
    if positions.size == 2 * size_x * size_y:
        SpectralRadar.createFreeformScanPattern.argtypes = [ProbeHandle,
                                                            ndpointer(dtype=np.float32, ndim=1, flags='C_CONTIGUOUS'),
                                                            C.c_int, C.c_int, BOOL]
...
def getScanPatternLUT(Pattern, PosX, PosY):
    ...
    scanner LUT.
    '''
    SpectralRadar.getScanPatternLUT.argtypes = [ScanPatternHandle,
...
def copyComplexDataContent(ComplexDataSource, DataContent):
    ...
    '''
    SpectralRadar.copyComplexDataContent.argtypes = [ComplexDataHandle,
                                                     ndpointer(dtype=np.complex64, ndim=3, flags='C_CONTIGUOUS')]
...
def copyRawDataContent(RawDataSource, DataContent):
    ...
    to call the original function.
    '''
    SpectralRadar.copyRawDataContent.argtypes = [RawDataHandle,
                                                 ndpointer(dtype=np.uint16, ndim=3, flags='C_CONTIGUOUS')]
